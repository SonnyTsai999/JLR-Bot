...
function selectBlocksByKeywords(query, defaultBlocks) {
  ...
  const chosen = [...new Set(defaultBlocks)];
...
export function selectBlocksWithConfidence(intentBlocks, chunks, options = {}) {
  const { min_confidence = 0.25, core_blocks = ['summary', 'key_findings', 'sources'] } = options;
  const { meanScore, ok } = computeEvidenceConfidence(chunks, { min_confidence });

  if (ok) {
    const withSources = intentBlocks.includes('sources') ? intentBlocks : [...intentBlocks, 'sources'];
    return withSources;
  }
  return core_blocks;
}
