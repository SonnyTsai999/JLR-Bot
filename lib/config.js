...
const DEFAULTS = {
  ...
  retrieval: { top_k: 12, max_chunks_per_source: 2, min_similarity_threshold: 0.0 },
...
export function getConfig() {
  ...
  const yAdaptive = y.adaptive_rag || {};
  return {
    embedding: {
    ...
    /** Public HTTPS URL to reddit_index.json (~100MB+). Local file is gitignored; use this on Vercel. */
    reddit_index_url: process.env.REDDIT_INDEX_URL || y.reddit_index_url || DEFAULTS.reddit_index_url,
    adaptive_rag: {
      use_intent_classification: yAdaptive.use_intent_classification !== false,
      min_confidence: yAdaptive.min_confidence ?? 0.25,
    },
  };
}
