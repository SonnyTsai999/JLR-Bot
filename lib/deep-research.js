...
import { isNarrowFactualQuery, narrowQueryFocusBlock } from './query-modes.js';
...
async function researchPlannerAgent(query, options = {}) {
  ...
  const narrowHint = narrow
    ? '\nNARROW QUESTION: scope and sub_questions must stay strictly on the user\'s exact ask (e.g. criteria of VIKOR in AEC). Do not broaden to generic surveys of AI, BIM, or sustainability unless the question asks for them.'
    : '';
...
Output only valid JSON, no other text.${narrowHint}`;
...
async function evidenceExtractionAgent(chunks, query, options = {}) {
  ...
  const narrow = options.narrow_query === true;
  const narrowFindingsHint = narrow
...
export async function runDeepResearch(query, options = {}) {
  ...
  const opts = { config, api_key: apiKey, narrow_query, model: options.model };
