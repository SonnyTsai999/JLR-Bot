...
export async function loadIndex(config = defaultConfig) {
  ...
  // Resolve index relative to project root (lib/ is under project root); works on Vercel where cwd may differ
  const projectRoot = pathMod.resolve(pathMod.dirname(fileURLToPath(import.meta.url)), '..');
  const fullPath = path
    ? pathMod.resolve(projectRoot, path)
    : pathMod.join(projectRoot, 'index', 'node_index.json');
