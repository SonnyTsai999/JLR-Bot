...
export async function loadIndex(config = defaultConfig) {
  ...
  const fullPath = path
    ? pathMod.resolve(process.cwd(), path)
    : pathMod.resolve(process.cwd(), 'index', 'node_index.json');
  if (!fs.existsSync(fullPath)) {
    throw new Error(
      path
        ? `Index file not found: ${fullPath}`
  ...
  return cachedIndex;
  throw new Error('No index available. Set INDEX_URL or INDEX_PATH, or place index/node_index.json (run: npm run export-index).');
}
...
export async function retrieve(query, options = {}) {
  ...
  const sourceCount = {};
  const results = [];
  const searchK = Math.min(withScore.length, Math.max(top_k * 3, 50));
